      ...
      <div className='w-fit h-fit fixed bottom-10 right-4 sm:right-[calc((100%-40rem)/2+1rem)] md:right-[calc((100%-48rem)/2+1rem)] lg:right-[calc((100%-64rem)/2+1rem)] xl:right-[calc((100%-80rem)/2+1rem)] 2xl:right-[calc((100%-96rem)/2+1rem)] z-40 grid place-items-center gap-2 max-sm:w-full'>
        <div className='flex items-center justify-end gap-2 w-full'>
          <motion.div initial={{ y: 0, opacity: 0, display: 'block' }} animate={{ y: popUp ? 20 : 0, opacity: popUp ? 0 : 100, display: popUp ? 'none' : 'block' }} transition={{ delay: 0.1 }} className='bg-white rounded-md py-1.5 px-3 cursor-pointer drop-shadow-xl' onClick={handlePopUp}>
            <p className='text-black text-sm'>Need Help? <strong className='font-semibold'>Chat with us</strong></p>
          </motion.div>
          <div>
            <div className='drop-shadow-xl rounded-full bg-[#2DB742] p-1 h-14 w-14 grid place-items-center cursor-pointer relative' onClick={handlePopUp}>
              <motion.span className=' absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2' initial={{ scale: 1, rotate: 0 }} animate={{ scale: popUp ? 0 : 1, rotate: popUp ? -180 : 0 }} transition={{ duration: 0.2 }}><FontAwesomeIcon icon={faWhatsapp} className='text-white text-4xl' size={32} /></motion.span>
              <motion.span className=' absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2' initial={{ scale: 0, rotate: 0 }} animate={{ scale: popUp ? 1 : 0, rotate: popUp ? 180 : 0 }} transition={{ duration: 0.2 }}><FontAwesomeIcon icon={faPlus} className='text-white text-4xl font-thin rotate-45' size={32} /></motion.span>
            </div>
          </div>
        </div>
        <motion.div initial={{ y: 0, opacity: 0, display: 'none' }} animate={{ y: popUp ? 0 : 50, opacity: popUp ? 100 : 0, display: popUp ? 'block' : 'none' }} transition={{ duration: 0.3 }} className='drop-shadow-xl rounded-sm overflow-hidden w-11/12 sm:min-w-88 absolute bottom-16 mb-3 right-0 -z-30'>
          <div>
            <div className='flex items-start justify-items-start bg-[#2DB742] text-white px-3.5 py-3.5 rounded-t-md'>
              <div className='rounded-full bg-[#2DB742] p-1 h-14 w-14 grid place-items-center cursor-pointer'>
                <FontAwesomeIcon icon={faWhatsapp} className='text-white text-4xl' size={36} />
              </div>
              <div>
            ...
            <div className='bg-gray-50 rounded-b-md px-4 py-3'>
              <div><p className='text-xs text-gray-400 p-1'>The team typically replies in a few minutes.</p></div>
                ...
                <Link href='/'>
                  <div className='p-2 border-l-2 border-[#2DB742] flex items-center justify-between gap-2 mt-3 rounded-sm'>
                    <div className='flex items-center gap-2'>
                      <div className='rounded-full bg-gray-50 p-1 h-12 w-12 grid place-items-center'>
                        <Image src='/logo/whatsapp-icon.svg' alt='Whatsapp' width={48} height={48} className='text-white text-4xl' />
                      </div>
